...
import 'package:task_manager/domain/enums/task_sort.dart';
...
void main() {
  ...
  group('TaskProvider', () {
    ...
    test('search filters tasks by title', () async {
      await provider.addTask(Task(title: 'Buy milk', priority: 'Low'));
      await provider.addTask(Task(title: 'Walk dog', priority: 'Low'));
    ...
    test('sort by priority high to low', () async {
      await provider.addTask(Task(title: 'Low task', priority: 'Low'));
      await provider.addTask(Task(title: 'High task', priority: 'High'));
      await provider.addTask(Task(title: 'Medium task', priority: 'Medium'));

      provider.setSort(TaskSort.priorityHighToLow);

      expect(provider.filteredTasks.map((t) => t.title).toList(), [
        'High task',
        'Medium task',
        'Low task',
      ]);
    ...
    test('undo restores deleted task', () async {
      final task = Task(id: '99', title: 'Undo me', priority: 'Low');
      await provider.addTask(task);
      await provider.deleteTask('99');
      ...
      expect(provider.tasks.first.title, 'Undo me');
    });
